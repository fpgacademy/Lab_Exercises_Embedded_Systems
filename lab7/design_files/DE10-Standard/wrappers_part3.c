...
/* This file contains wrappers for /dev/accel */
int accel_FD = -1;

#define accel_CHAR_DEV "/dev/accel"

#define ACCEL_BYTES 20				// number of bytes to read from accel device
...
int accel_read(int *ready, int *x, int *y, int *z, int *mg_per_lsb)
{
	...
	if (accel_FD > 0)
	{
		...
		else if (sscanf (accel_buffer, "%X %d %d %d %d", ready, x, y, z, mg_per_lsb) != 5)
		{
